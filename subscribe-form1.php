...
$sql_form_1 = "SELECT SubjectCode, SubjectName, subjectImage, SubjectPrice FROM subject WHERE Form = 'F1'";
$result_form_1 = $conn->query($sql_form_1);

// Array to store fetched subjects
$subjects_form_1 = array();

if ($result_form_1->num_rows > 0) {
    while ($row = $result_form_1->fetch_assoc()) {
        $subjects_form_1[] = $row;
    }
}
...
<body>
    ...
    <div class="main_content">
        ...
        <div class="row">
            <?php 
            $count = 0;
            foreach ($subjects_form_1 as $subject) : 
                $count++;
            ?>
                <div class="card">
                    <div class="card-body">
                    <a href="cart-page.php?subjectCode=<?php echo $subject['SubjectCode']; ?>&form=1">
                        <img src="<?php echo $subject['subjectImage']; ?>" alt="<?php echo $subject['SubjectName']; ?>">
                    </a>
                    </div>
                </div>
                <?php if ($count % 3 == 0 && $count < count($subjects_form_1)) : ?>
                    </div><div class="row">
                <?php endif; ?>
            <?php endforeach; ?>
        </div>
